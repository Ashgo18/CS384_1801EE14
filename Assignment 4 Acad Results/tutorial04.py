    ...
    if file != 'misc.csv':
        ...
        with open(cd1,'r') as f:
            ...
            for row in data:
                x=x+1
                if(x>3):
                    if grade_obtn_per_sem.get(row[4]) == None:      #if particular sem is not in dict then
                        credit_clrd_per_sem[row[4]]=0               #row[4] is a sem number
                        total_credit_per_sem[row[4]]=0
                        grade_obtn_per_sem[row[4]]=[]
                        corresponding_credit[row[4]]=[]
            
                    grade_obtn_per_sem[row[4]].append(grade_map[row[3]])        #row[3] is grade to course
        ...
        with open(cd1,'w',newline='') as f:
            ...
            for sem in range(1,total_sems+1):
                ...
                for x in range(0,len(grade_obtn_per_sem[semester])):
                    spi=spi+grade_obtn_per_sem[semester][x]*corresponding_credit[semester][x]
                ...
                data.writerow([semester,total_credit_per_sem[semester],credit_clrd_per_sem[semester],round(spi,2),tot_credits,tot_credit_clrd,round(cpi,2)])
