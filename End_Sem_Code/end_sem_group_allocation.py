...
def group_allocation(filename, number_of_groups):
    ...
    with open(temp_file, 'r') as file:
        ...
        with open('stats_grouping.csv','w',newline='') as file:
            data = csv.writer(file)
        ...
        for key in branch_total:
            ...
            with open(cdd,'r') as file:
                ...
                temp += 1
                for i in range(number_of_groups):
                    curr_file = os.path.join(cd,groups_stats_info[0][i]+'.csv')
...
number_of_groups = int(input("Enter the number of groups in which students have to be divided:- "))
